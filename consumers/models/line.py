...
class Line:
    ...
    def process_message(self, message):
        ...
        topic = message.topic()
        if topic == "org.chicago.cta.stations.table.v1":  # Faust topic
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "org.chicago.cta.station.arrivals" in topic:  # Arrivals topic
            self._handle_arrival(message)
        elif topic == "TURNSTILE_SUMMARY":  # Ksql topic
